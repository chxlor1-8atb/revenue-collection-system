          ...
          <div className="flex flex-wrap items-center gap-2 flex-1">
            ...
            <div className="relative flex-1 min-w-[240px] max-w-md">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-2 bg-white border border-slate-200 rounded-xl text-xs placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#5B58F2]/30 focus:border-[#5B58F2]"
              />
              {searchTerm && (
                <button
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto py-1">
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                  filterType === "all" ? "bg-[#5B58F2] text-white shadow-2xs" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-100"
                }`}
              >
                ทั้งหมด
              </button>
          ...
          {activeTab === "pending" && filteredSlips.length > 0 && (
            <div className="flex items-center gap-2 self-end md:self-center">
              <button
                ...
                className="text-xs font-semibold text-slate-600 hover:text-[#5B58F2] flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 transition-colors cursor-pointer"
              >
                {selectedSlipIds.length === filteredSlips.length && filteredSlips.length > 0 ? (
                  <><CheckSquare size={14} className="text-[#5B58F2]" /> ยกเลิกเลือกทั้งหมด</>
                ) : (
                  ...
                  return (
                    <div 
                      ...
                      className={`bg-white border rounded-3xl overflow-hidden flex flex-col justify-between shadow-2xs hover:shadow-md transition-all group relative ${
                        isSelected ? "border-[#5B58F2] ring-2 ring-[#5B58F2]/20" : "border-slate-200/90 hover:border-[#5B58F2]/40"
                      }`}
                    >
                      ...
                      {activeTab === "pending" && (
                        <div className="absolute top-3 left-3 z-20">
                          <button
                            ...
                            className="w-7 h-7 rounded-lg bg-white/90 backdrop-blur-md border border-slate-200/80 flex items-center justify-center text-slate-600 hover:text-[#5B58F2] shadow-2xs transition-transform active:scale-95 cursor-pointer"
                          >
                            {isSelected ? (
                              <CheckSquare size={16} className="text-[#5B58F2]" />
                            ) : (
                              <Square size={16} />
                            )}
                          </button>
                      ...
                      {activeTab === "pending" && hasSmartMatch && (
                        <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border-b border-purple-100 p-2.5 flex items-center justify-between gap-2">
                          <div className="flex items-center gap-1.5 min-w-0">
                            <Sparkles size={13} className="text-[#5B58F2] shrink-0" />
                            <div className="truncate text-xs">
                              <span className="font-bold text-[#5B58F2]">แนะนำ: บ้าน {slip.smartMatch.house.houseNumber}</span>
                          ...
                          <button
                            ...
                            className="bg-[#5B58F2] hover:bg-[#4A47D1] text-white px-2.5 py-1 rounded-lg text-[11px] font-bold shadow-2xs shrink-0 cursor-pointer flex items-center gap-1"
                          >
                        ...
                        <tr key={slip.id} className={`hover:bg-slate-50/80 transition-colors ${isSelected ? "bg-indigo-50/30" : ""}`}>
                          {activeTab === "pending" && (
                            <td className="px-4 py-4">
                              <button
                                onClick={() => toggleSelectSlip(slip.id)}
                                className="text-slate-400 hover:text-[#5B58F2] cursor-pointer"
                              >
                                {isSelected ? <CheckSquare size={16} className="text-[#5B58F2]" /> : <Square size={16} />}
                              </button>
                            </td>
                          )}
                          ...
                          <td className="px-5 py-4">
                            {slip.houseNumber ? (
                              ...
                              <span className="inline-flex items-center gap-1 bg-purple-50 text-[#5B58F2] border border-purple-200 text-xs font-bold px-2 py-0.5 rounded-lg">
      ...
      {selectedSlipIds.length > 0 && activeTab === "pending" && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-slate-900/95 text-white px-6 py-3.5 rounded-2xl shadow-2xl backdrop-blur-md flex items-center gap-4 border border-slate-700/60 animate-in slide-in-from-bottom-5">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-1.5 bg-[#5B58F2] hover:bg-[#4A47D1] text-white px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-md shadow-[#5B58F2]/30 cursor-pointer disabled:opacity-50"
          >
                    ...
                    <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200/70 space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-slate-700 flex items-center gap-1.5">
                          <Calendar size={14} className="text-[#5B58F2]" /> จ่ายล่วงหน้า (งวดถัดไป):
                        </span>
                        <span className="font-bold text-[#5B58F2] font-mono">
            ...
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/70 flex flex-col sm:flex-row justify-between items-center gap-3">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                <button 
                  ...
                  className="flex-1 sm:flex-initial px-6 py-2.5 bg-[#5B58F2] hover:bg-[#4A47D1] text-white rounded-xl transition-all font-bold text-xs disabled:opacity-50 shadow-md shadow-[#5B58F2]/25 flex items-center justify-center gap-2 cursor-pointer"
                >
